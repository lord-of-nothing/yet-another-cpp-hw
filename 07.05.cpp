 ...
 public:
  /* explicit */ Ratio(int num = 0, int den = 1) : m_num(num), m_den(den) {
    if (m_den == 0) {
      ...
      throw std::invalid_argument("Ratio::Ration : invalid denominator");
    }
 ...
 public:
  friend auto& operator>>(std::istream& stream, Ratio& ratio) {
    ...
    if (c != '/') {
      ...
      throw std::runtime_error("operator>> : invalid input");
    }
  ...
  auto& operator-=(const Ratio& other) { return *this += other.m_num * -1; }
...
int main() {
  // что-то не очень понял, как засунуть конструктор в try-catch
  // и при этом иметь возможность пользоваться объектом в дальнейшем
  // или я туплю, и этого и не надо было делать?
  //  try {
